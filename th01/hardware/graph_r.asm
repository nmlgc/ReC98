...
public _z_grcg_boxfill
_z_grcg_boxfill	proc far

@@vram		= dword	ptr -0Eh
@@right_pixels		= byte ptr -0Ah
@@left_pixels		= byte ptr -9
@@tmp		= word ptr -8
@@full_width_byte_count		= word ptr -6
@@y		= word ptr -4
@@x		= word ptr -2
@@left		= word ptr  6
@@top		= word ptr  8
...
@@put:
	...
	sar	dx, 3
	add	ax, dx
	mov	word ptr [bp+@@vram+2], 0A800h
	mov	word ptr [bp+@@vram], ax
	mov	[bp+@@y], si
	jmp	short @@yloop_more?
; ---------------------------------------------------------------------------
...
@@yloop:
	...
	sar	dx, 3
	sub	ax, dx
	mov	[bp+@@full_width_byte_count], ax
	mov	cl, byte ptr [bp+@@left]
	and	cl, 7
	mov	ax, 0FFh
	sar	ax, cl
	...
	shl	al, cl
	mov	[bp+@@right_pixels], al
	cmp	[bp+@@full_width_byte_count], 0
	jnz	short @@at_least_one_full_byte
	mov	al, [bp+@@left_pixels]
	and	al, [bp+@@right_pixels]
	les	bx, [bp+@@vram]
...
@@xloop_more?:
	mov	ax, [bp+@@x]
	cmp	ax, [bp+@@full_width_byte_count]
	jl	short @@xloop
	les	bx, [bp+@@vram]
	add	bx, [bp+@@full_width_byte_count]
	mov	al, [bp+@@right_pixels]

@@next_row:
	mov	es:[bx], al
	add	word ptr [bp+@@vram], ROW_SIZE
	inc	[bp+@@y]
...
@@ret:
	...
	leave
	retf
_z_grcg_boxfill	endp


public _graph_r_box
_graph_r_box	proc far
...
@@col		= word ptr  0Eh
	...
	mov	di, [bp+@@left]
	mov	si, [bp+@@col]
	call	_graph_r_hline stdcall, di, [bp+@@right], [bp+@@top], si
	call	_graph_r_vline stdcall, di, [bp+@@top], [bp+@@bottom], si
	call	_graph_r_vline stdcall, [bp+@@right], [bp+@@top], [bp+@@bottom], si
	call	_graph_r_hline stdcall, di, [bp+@@right], [bp+@@bottom], si
	add	sp, 20h
	pop	di
	pop	si
	pop	bp
