...
; TASM doesn't seem to support segment prefixes in these macros, and somehow
; parses them as duplicated definitions (???), resulting in a "symbol already
; defined elsewhere" error...

; for pi decode
	xwidth		EQU word ptr [BUFF_LEN]
	ywidth		EQU word ptr [BUFF_LEN+2]
	gbuffer		EQU dword ptr [BUFF_LEN+4]
	gbuffer_off	EQU word ptr [BUFF_LEN+4]
	gbuffer_seg	EQU word ptr [BUFF_LEN+6]
	gblast		EQU dword ptr [BUFF_LEN+8]
	gblast_off	EQU word ptr [BUFF_LEN+8]
	gblast_seg	EQU word ptr [BUFF_LEN+10]
	ctable		EQU		 BUFF_LEN+12
	mode		EQU byte ptr [BUFF_LEN+256+12]

; for bit_load
	handle		EQU word ptr [BUFF_LEN+256+14]
	buff_p		EQU word ptr [BUFF_LEN+256+16]
	bit_buf		EQU byte ptr [BUFF_LEN+256+18]
	bit_len		EQU byte ptr [BUFF_LEN+256+19]
...
@@MACHINE_INFORMATION:
	...
	nopcall	HMEM_LALLOCATE	; minfo buffer
	pop	DS
	stosw		; 拡張データ segment
...
;GBUFFER ALLOCATE
	...
	mov	DS:gblast_off,AX	; 終了アドレス
	mov	DS:gblast_seg,DX
	push	seg DGROUP
	pop	DS
	mov     mem_AllocID,MEMID_pi
	nopcall	HMEM_LALLOCATE		; 画像領域の確保
	pop	DS
	mov	BX,AX
...
@@SET_PTRS:
	...
	add	DS:gblast_seg,AX	; add top address (segment)
	mov	CX,DS:xwidth
	;add	CX,gbuffer_off
	_push	ES
	_les	BX,[BP+@@bufptr]
...
even
_end_checkB:
	cmp	DS:gblast_off,DI
	jbe	short _end_checkC
	jmp	_main_decode_loopA
_end_checkC:
	mov	AX,ES
	cmp	DS:gblast_seg,AX
	jbe	short @@file_close
	jmp	_main_decode_loopA
...
@@file_close:
	...
	mov	BX,seg DGROUP
	mov	DS,BX
	nopcall	SMEM_RELEASE		; ←全レジスタ保存だから
