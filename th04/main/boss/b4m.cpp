...
bool pascal near marisa_flystep_pointreflected(int duration)
{
	...
	if(flystep_pointreflected_frame == 0) {
		// Mod: Prevent the original function's division by zero, and move
		// Marisa to the target point at the maximum possible initial positive
		// speed instead. This causes the braking phase to already start on the
