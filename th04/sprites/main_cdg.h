...
#if (GAME == 5)
	// TH05 only uses the CDG_FACESET_PLAYCHAR and CDG_FACESET_BOSS ranges as
	// temporary storage for EMS area preloading. During in-game dialog, every
	// single rendered face image is individually loaded to this one slot,
	// either from the EMS area or from disk, and then immediately freed.
