...
@@strlen:
	inc	si
	cmp	snd_load_fn[si], 0
	jnz	short @@strlen
	mov	(snd_load_fn+4)[si], 0
	mov	snd_load_fn[si], '.'
	inc	si
	cmp	[bp+@@func], SND_LOAD_SE
	jnz	short @@song
	mov	snd_load_fn[si], 'e'
	mov	(snd_load_fn+1)[si], 'f'
	cmp	snd_se_mode, SND_SE_OFF
	jz	@@ret
	cmp	snd_se_mode, SND_SE_BEEP
	jnz	short @@FM_SE
	mov	(snd_load_fn+2)[si], 's'
	call	bgm_read_sdata pascal, ds, offset snd_load_fn
	jmp	@@ret

@@FM_SE:
	mov	(snd_load_fn+2)[si], 'c'
	jmp	short @@open
