...
static int saveout_write_prologue(struct rec98_bmp2arr_task *t,FILE *fp,int sscols,int ssrows,int bytesperrow) {
...
static int saveout_write_epilogue(struct rec98_bmp2arr_task *t,FILE *fp,int sscols,int ssrows,int bytesperrow) {
    if (t->output_type == REC98_OUT_C) {
        fprintf(fp,"};\n");
    }
    else if (t->output_type == REC98_OUT_ASM) {
...
int rec98_bmp2arr_save_output(struct rec98_bmp2arr_task *t) {
    ...
    if (saveout_write_prologue(t,fp,sscols,ssrows,bytesperrow))
        goto fioerr;

    // TODO

    if (saveout_write_epilogue(t,fp,sscols,ssrows,bytesperrow))
        goto fioerr;
