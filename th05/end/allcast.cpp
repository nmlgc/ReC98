...
)
{
	...
	for(int i = 0; i < (PI_MASK_COUNT * PAGE_COUNT); i++) {
		// ZUN quirk: Dividing by [PI_MASK_COUNT] reduces this animation to
		// showing only the first [PAGE_COUNT] (i.e., 2) masks at an interval
		// of 4 double frames, rather than the probably expected full 4 masks
		// at a 2-double-frame interval.
		pi_put_quarter_masked_8(
			pic_left, pic_top, 0, quarter, (i / int(PI_MASK_COUNT))
		);
		wait_flip_and_check_measure_target();
	}
	...
	// Loading the .PI image for the next screen in front of an intentional
	// delay. Placing this before the 30-measure delay below would have been
	// even better, but this is still nice. (Except for the [SCREEN_COUNT]
	// landmine documented above.)
	loaded_screen_id++;
